#!/usr/bin/env python3
"""
Railway-ready Simmer FastLoop bot (v6.1 - Iron-Clad Full Edition).

CHANGELOG:
- ✅ RESTORED SHORTCUTS: -l, -q, -s now work as expected.
- ✅ MOMENTUM FIX: Correctly handles negative momentum for "NO" trades.
- ✅ IRON-CLAD CLOSE: Double-checks wallet balance via API. No ghost closes.
- ✅ CONVENIENCE: Settings at the top for lightning-fast updates.
"""
...
TAKE_PROFIT_PCT = 0.15       # Take Profit (0.20 = 20%)
...
def api_request(url, method="GET", data=None, headers=None, timeout=25):
    try:
        ...
        req_headers.setdefault("User-Agent", "railway-fastloop/6.1")
...
def monitor_and_close(api_key, market_id, end_time, side):
    ...
    print(f"MONITOR: Tracking {side.upper()}. Entry: {entry_price:.3f}.")
...
def run_once(live, quiet, smart_sizing):
    ...
    # MOMENTUM CHECK: Handle absolute value for threshold
